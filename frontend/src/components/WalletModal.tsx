...
const WALLETS = [
  { key: "leo", label: "Leo Wallet", emoji: "🦁", color: "rgba(59,130,246,0.15)", availKey: "leoAvailable" },
  { key: "puzzle", label: "Puzzle Wallet", emoji: "🧩", color: "rgba(168,85,247,0.15)", availKey: "puzzleAvailable" },
  { key: "shield", label: "Shield Wallet", emoji: "🛡️", color: "rgba(34,197,94,0.15)", availKey: "shieldAvailable" },
];

export function WalletModal({ onClose }: { onClose: () => void }) {
  const { connectWithPuzzle, connectWithLeo, connectWithShield, puzzleAvailable, leoAvailable, shieldAvailable, connecting, error } = useWallet();
  const [mounted, setMounted] = useState(false);

  useEffect(() => { setMounted(true); }, []);

  const availability: Record<string, boolean> = { leoAvailable, puzzleAvailable, shieldAvailable };

  const handleConnect = async (type: string) => {
    try {
      if (type === "puzzle") await connectWithPuzzle();
      else if (type === "leo") await connectWithLeo();
      else if (type === "shield") await connectWithShield();
      if (!error) onClose();
    } catch {}
  };
  ...
  const modal = (
    ...
    >
      <motion.div
        initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {WALLETS.map(({ key, label, emoji, color, availKey }) => {
            const available = availability[availKey];
            return (
              <button
                key={key}
        ...
        {connecting && <p style={{ textAlign: "center", color: "#00ff88", fontSize: "14px", marginTop: "16px", fontFamily: "Space Grotesk, sans-serif" }}>Connecting — check your wallet extension...</p>}
        {error && <p style={{ textAlign: "center", color: "rgba(255,100,100,0.8)", fontSize: "12px", marginTop: "12px", fontFamily: "Space Grotesk, sans-serif", lineHeight: 1.5 }}>{error}</p>}

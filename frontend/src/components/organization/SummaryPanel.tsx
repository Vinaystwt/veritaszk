  ...
  return (
    <div style={{ position: "sticky", top: "88px" }}>
          ...
          <AnimatePresence>
            {assets.map((a) => (
              <motion.div
                ...
                style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", borderRadius: "8px", background: "rgba(255,255,255,0.03)", marginBottom: "6px", overflow: "hidden" }}
              >
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: ASSET_COLORS[a.asset_type], flexShrink: 0 }} />
          ...
          <AnimatePresence>
            {liabilities.map((l) => (
              <motion.div
                ...
                style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", borderRadius: "8px", background: "rgba(255,255,255,0.03)", marginBottom: "6px", overflow: "hidden" }}
              >
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: ASSET_COLORS[l.liability_type], flexShrink: 0 }} />
      ...
      >
        ...
        {proofStatus === "none" && <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "14px" }}>No active proof. Generate one below.</p>}

        {proofStatus === "active" && (
          <>
            <motion.div animate={{ textShadow: ["0 0 10px rgba(0,255,136,0.3)", "0 0 25px rgba(0,255,136,0.6)", "0 0 10px rgba(0,255,136,0.3)"] }} transition={{ duration: 2, repeat: Infinity }}
              style={{ fontSize: "28px", fontWeight: 700, color: "#00ff88", marginBottom: "12px" }}>
              SOLVENT ✓
            </motion.div>
            {proofTimestamp && <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", marginBottom: "8px" }}>Generated: {proofTimestamp}</p>}
            {proofNonce && <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", fontFamily: "monospace", wordBreak: "break-all", marginBottom: "16px" }}>Nonce: {proofNonce.slice(0, 20)}...</p>}
        ...
        {proofStatus === "revoked" && <div style={{ fontSize: "20px", fontWeight: 700, color: "#ff4444" }}>REVOKED</div>}
      </motion.div>
    </div>
  );
}

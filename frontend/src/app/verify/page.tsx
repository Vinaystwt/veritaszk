...
function VerifyContent() {
  ...
  const saveRecent = (address: string, status: VerificationStatus) => {
    const updated = [
      { address, status, timestamp: Date.now() },
      ...recentLookups.filter((r) => r.address !== address),
    ].slice(0, 5);
  ...
  // Deep-link auto-fill + auto-verify
  useEffect(() => {
        ...
        <AnimatePresence mode="wait">
          {result && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -16 }}
              style={{ padding: "32px", borderRadius: "16px", border: `1px solid ${statusColor[result.status]}33`, background: `${statusColor[result.status]}08`, marginBottom: "32px" }}
            >
              {result.status === "SOLVENT" && (
                <>
                  ...
                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                    <button
                      onClick={copyUrl}
                      style={{ flex: 1, padding: "10px 16px", borderRadius: "8px", background: "#00ff88", color: "#0a0a0f", fontWeight: 600, fontSize: "13px", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", fontFamily: "Space Grotesk, sans-serif" }}
                    >
                      <Copy size={13} />
                      {copied ? "Copied!" : "Share Verification"}
                    </button>
                    <button
                      onClick={() => window.open(`https://explorer.aleo.org/testnet/address/${input}`, "_blank")}
                      style={{ flex: 1, padding: "10px 16px", borderRadius: "8px", background: "transparent", color: "rgba(255,255,255,0.5)", fontSize: "13px", border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", fontFamily: "Space Grotesk, sans-serif" }}
                    >
              ...
              {result.status === "NOT_FOUND" && (
                <>
                  ...
                  <button
                    onClick={() => handleVerify()}
                    style={{ padding: "10px 20px", borderRadius: "8px", background: "rgba(255,68,68,0.1)", border: "1px solid rgba(255,68,68,0.3)", color: "#ff4444", fontSize: "13px", cursor: "pointer", fontFamily: "Space Grotesk, sans-serif" }}
                  >
                    Retry
                  </button>
                </>

...
const STATUS_COLOR: Record<string, string> = { SOLVENT: "#00ff88", REVOKED: "#f97316", UNVERIFIED: "rgba(255,255,255,0.3)" };
const TOKEN_COLORS: Record<string, string> = { ALEO: "#6366f1", USDCx: "#22c55e", USAD: "#eab308" };
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", padding: "40px 24px 80px" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: "40px" }}>
          <div style={{ display: "inline-block", padding: "4px 12px", borderRadius: "100px", border: "1px solid rgba(0,255,136,0.3)", background: "rgba(0,255,136,0.05)", color: "#00ff88", fontSize: "12px", marginBottom: "16px" }}>Public Dashboard</div>
        ...
        >
          {[
            ...
            <div key={s.label} style={{ padding: "24px", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", backdropFilter: "blur(12px)", textAlign: "center" }}>
              <div style={{ fontSize: "40px", fontWeight: 700, color: "#00ff88", marginBottom: "8px" }}>
                {loading ? "—" : <AnimatedNumber target={s.value} />}
              </div>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 12px", borderRadius: "8px", border: "1px solid rgba(0,255,136,0.2)", background: "rgba(0,255,136,0.04)" }}>
              <motion.div animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#00ff88" }} />
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: "20px" }}>
            {filtered.map((org, i) => (
              <motion.div key={org.address}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.05 }}
                whileHover={{ y: -2, borderColor: `${STATUS_COLOR[org.status]}44` }}
                style={{
                  padding: "28px",
                  borderRadius: "16px",
                  border: "1px solid rgba(255,255,255,0.08)",
                  borderLeft: `3px solid ${STATUS_COLOR[org.status]}`,
              ...
              >
                ...
                <div style={{ display: "flex", gap: "6px", marginBottom: "20px" }}>
                  {["ALEO", "USDCx", "USAD"].map((token) => (
                    <span key={token} style={{ padding: "3px 8px", borderRadius: "4px", fontSize: "11px", background: `${TOKEN_COLORS[token]}15`, color: TOKEN_COLORS[token], border: `1px solid ${TOKEN_COLORS[token]}30` }}>{token}</span>
                  ))}
                </div>
                <button onClick={() => router.push(`/verify?address=${org.address}`)}

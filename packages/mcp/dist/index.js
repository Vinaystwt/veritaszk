...
async function queryIndexer(path, indexerUrl) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'check_org_solvency') {
        const c = args?.commitment_or_name;
        const idxUrl = args?.indexer_url;
        const data = await queryIndexer(`/api/proofs/${c}`, idxUrl);
        if (!data) {
            return { content: [{ type: 'text', text: `Could not fetch solvency data for ${c}. Is the indexer running?` }] };
        }
        const tierNames = {
    ...
    if (name === 'list_expiring_proofs') {
        const withinBlocks = args?.within_blocks;
        const idxUrl = args?.indexer_url;
        const all = await queryIndexer('/api/proofs', idxUrl);
        if (!all || !Array.isArray(all)) {
            return { content: [{ type: 'text', text: 'Could not fetch proof list. Is the indexer running?' }] };
        }
        ...
        const expiring = all
            .filter((p) => p.expiry > 0 && !p.isExpired)
            .map((p) => ({
            ...
            .filter((p) => p.blocksRemaining <= withinBlocks);
